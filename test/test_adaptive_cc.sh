...
PY="${PY:-python3}"
PREFIX="${PREFIX:-/tmp/acctest2}"; KEY="${API_KEY:-}"
...
http {
  ...
  init_worker_by_lua_block {
    ...
    -- ⚠️ 显式钉住派生系数(AC6 用):不钉的话会继承 session_base.conf 的当前值,生产一改测试就红。
    --    这里取 0.4(= 2026-08 生产值);只有 AC6 的 acd 路由依赖派生 min,别的路由都显式配了 adaptive_cc_min。
    _G.ADAPTIVE_CC_MIN_FRAC = 0.4
...
#   不配 min → 派生 = 静态max(40) × ADAPTIVE_CC_MIN_FRAC(init block 里钉死 0.4)= 16,max=40
...
{ [ "$on" = "True" ] && [ "$mn_" = "16" ] && [ "$mx_" = "40" ]; } \
  && ok "AC6 默认开(只配tps_limit_tps→on=$on)+派生min=$mn_(=40*0.4),max=$mx_" || no "AC6 on=$on min=$mn_ max=$mx_"
