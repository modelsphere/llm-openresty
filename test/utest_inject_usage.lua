...
-- 只测该函数,故直接内联被测逻辑的依赖:reqtransform.lua 顶层 require "util",先 stub package.loaded.util。
...
local rt = assert(loadfile(arg[1] or "lua/reqtransform.lua"))()  -- 用法: resty test/utest_inject_usage.lua [lua/reqtransform.lua]
...
local n1, inj1 = rt.surgical_inject_usage(b1, { stream = true })
...
local b1b = '{"a":1}\n'
local n1b = rt.surgical_inject_usage(b1b, { stream = true })
...
local n2, inj2 = rt.surgical_inject_usage(b2, { stream = true, stream_options = { foo = 1 } })
...
local n2b = rt.surgical_inject_usage(b2b, { stream = true, stream_options = {} })
...
local n3, inj3 = rt.surgical_inject_usage(b3, { stream = true, stream_options = { include_usage = false } })
...
local n3b, inj3b = rt.surgical_inject_usage(b3b, { stream = true, stream_options = { include_usage = cjson.null } })
...
local n3c, inj3c = rt.surgical_inject_usage(b3c, { stream = true, stream_options = { include_usage = 0 } })
...
local n3d = rt.surgical_inject_usage(b3d, { stream = true, stream_options = { include_usage = false } })
...
local n4 = rt.surgical_inject_usage(b4, { stream = true })
...
local n5, inj5 = rt.surgical_inject_usage(b5, { stream = true })
...
local n5b = rt.surgical_inject_usage(b5b, { stream = true })
...
local n6, inj6 = rt.surgical_inject_usage(b6, { stream = true })
